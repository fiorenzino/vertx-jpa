...
import nz.fiore.vertx.ext.jpa.base.JPATestBase;
import nz.fiore.vertx.ext.jpa.model.User;
...
public class JPATest extends JPATestBase
{
  ...
  protected JPAClient client;

  @Before
  public void setUp() throws Exception
  {
    super.setUp();
    client = JPAClient.createNonShared(vertx, config());
  }
  ...
  public void testPersist()
  {
    ...
    client.persist("users", user.toJson(), result -> {
      assertTrue(result.succeeded());
      assertEquals(1, result.result().getUpdated());
    });
  }
  ...
  @Test
  public void testRestrictionsQuery()
  {
  ...
  //            // syntax error :(
  //            conn.queryWithParams("SELECT LNAME FROM insert_table WHERE id = ?", new JsonArray().add(id),
  ...
  //        conn
  //          .queryWithParams("SElECT DOB FROM insert_table WHERE id=?;", new JsonArray().add(id), onSuccess(resultSet -> {
  //            assertNotNull(resultSet);
  //            assertEquals(1, resultSet.getResults().size());
