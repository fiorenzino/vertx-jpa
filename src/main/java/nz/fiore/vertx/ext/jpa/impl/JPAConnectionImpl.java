...
class JPAConnectionImpl implements JPAConnection
{
   ...
   public JPAConnection batchCallableWithParams(String statement, List<JsonArray> inArgs, List<JsonArray> outArgs,
            Handler<AsyncResult<List<Integer>>> handler)
   {
      new JDBCBatch(vertx, jdbchelper, options, ctx, statement, inArgs, outArgs)
               .execute(conn, statementsQueue, handler);
      return this;
   }
   ...
   @Override
   public JPAConnection find(String table, JsonObject key, Handler<AsyncResult<ResultSet>> resultHandler)
   {
      new JPAFind(vertx, helper, options, ctx, table, key).execute(conn, statementsQueue, resultHandler);
      return this;
   }

   @Override
   public Single<ResultSet> rxFind(String table, JsonObject key)
   {
      return new io.vertx.reactivex.core.impl.AsyncResultSingle<ResultSet>(handler -> {
